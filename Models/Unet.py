...
class Unet:
    ...
    def get_output(self, input, reuse=True):
        ...
        with tf1.variable_scope("separator", reuse=reuse):
            enc_outputs = list()

            current_layer = tf1.layers.conv2d(input, NUM_INITIAL_FILTERS, 3, activation=tf.nn.relu, padding='valid') # - 2
            ...
            for i in range(self.num_layers):
                ...
                current_layer = tf1.layers.max_pooling2d(current_layer, pool_size=2, strides=2, padding='valid', data_format='channels_last') # MAXPOOL # :2
                current_layer = tf1.layers.conv2d(current_layer, NUM_INITIAL_FILTERS * (2 ** (i+1)), 3, activation=tf.nn.relu, padding='valid') # CONV # -2
            ...
            for i in range(self.num_layers):
                ...
                current_layer = tf1.layers.conv2d_transpose(current_layer, NUM_INITIAL_FILTERS*(2**(self.num_layers-i-1)), 2, strides=2, activation=tf.nn.relu, padding='valid') # *2
                current_layer = Utils.crop_and_concat(enc_outputs[-i-1], current_layer) #tf.concat([enc_outputs[-i - 1], current_layer], axis=3)
                current_layer = tf1.layers.conv2d(current_layer, NUM_INITIAL_FILTERS * (2 ** (self.num_layers - i - 1)), 3, activation=tf.nn.relu, padding='valid') # - 2
            ...
            acc_norm = tf1.layers.conv2d(current_layer, 1, 1, activation=tf.nn.relu, padding='valid')  # 0
            voice_norm = tf1.layers.conv2d(current_layer, 1, 1, activation=tf.nn.relu, padding='valid')  # 0
